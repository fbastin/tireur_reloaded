...
| `start_charges.local.json` | `node scripts/build_start_charges.js` | the **starting-charge pre-fill** (typical bullet + real min charge, auto-filled when the cartridge/powder changes) |

The UI **degrades gracefully** when a `*.local.json` is absent (e.g. no starting-charge
pre-fill). On the **live site** these must be regenerated/deployed after data changes;
they are intentionally not in the public repo. Starting charges come **only** from
published minimum loads — never derived from the model, whose pressure under-prediction
would make a model-derived "max" charge unsafe.
